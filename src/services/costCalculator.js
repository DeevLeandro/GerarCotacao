...
export function getIOFUSD(lines) {
  ...
  for (const line of lines) {
    const name = line.name?.toLowerCase() || '';
    if (name === 'frete internacional') {
      frete = parseFloat(line.usd) || 0;
    }
  }
 ...
 * Soma tudo exceto a linha do ICMS Saída
 */
...
export function getLineUSD(line, allLines) {
  ...
  if (usd > 0 && !line.dependsOn) return usd;
  
  // IOF especial: 3.5% sobre frete
  if (name.includes('iof') && pct > 0) {
    return getIOFUSD(allLines);
  }
  
  // ICMS Saída especial: 17% sobre valor agregado
  if (name.includes('icms saída') && pct > 0) {
    return getICMSSaidaUSD(allLines);
  }
  
  // Impostos baseados no Valor Aduaneiro
  if (pct > 0 && (name.includes('icms entrada') || name.includes('ipi') || name.includes('pis') || name.includes('cofins') || name.includes('imposto de importação'))) {
    const valorAduaneiro = getValorAduaneiroUSD(allLines);
    return valorAduaneiro * pct / 100;
  }
  
  // Percentuais normais (sobre base padrão)
  if (pct > 0) {
...
export function getTotalBRL(lines, fx) {
  ...
  const fxRate = parseFloat(fx) || DEFAULT_FX;
...
/**
 * Aplica valor desejado ajustando a margem
 */
